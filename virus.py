...
# Can be customized to run for other Viruses as well
COVID19_PARAMS = {
    "r0": 2.28,
    "incubation": 5,
    "percent_mild": 0.8,
...
class Virus():
    def __init__(self, params):
        # create plot
        self.fig = plt.figure()
        self.axes = self.fig.add_subplot(111, projection="polar") # 1 by 1 grid
        ...
        self.num_deaths = 0
        self.r0 = params["r0"] # infections generated from single infection
        self.percent_mild = params["percent_mild"] # mild cases
        self.percent_severe = params["percent_severe"] # severe case
        self.fatality_rate = params["fatality_rate"]
        self.serial_interval = params["serial_interval"] # avg time between successive cases in chain transmission
        ...
        self.mild = {i: {"thetas": [], "rs": []} for i in range(self.mild_fast, 365)} # number of mild cases in the next year
        self.severe = {
            "recovery": {i: {"thetas": [], "rs": []} for i in range(self.severe_fast, 365)},
            "death": {i: {"thetas": [], "rs": []} for i in range(self.death_fast, 365)}
        }

        # restriction on how much the virus can spread in single wave
        self.exposed_before = 0 # amount of people exposed to virus already
        self.exposed_after = 1 # amount of people exposed to virus at end of current wave

        self.initial_population()

    def initial_population(self):
        population = 4500
        self.num_currently_infected = 1 # patient zero
    ...
    def spread_virus(self, i):
        self.exposed_before = self.exposed_after # beginning of new wave
        if self.day % self.serial_interval == 0 and self.exposed_before < 4500: # if day is a multiple of serial interval, we spread disease
